...
import { ViewAllFileIdHeader , ViewFileHeader, ViewAllFileIdResponse } from '@clone-google-drive/commons';
...
export const handleGetAllFileId = async (req: Request, res: Response) => {
    try {
        ...
        if (files) {
            const responseData: ViewAllFileIdResponse = files.map((file: FileModel) => {
                return {
                    id: file.id,
